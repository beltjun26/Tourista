<?php
	...
	if(isset($_POST['change_profile'])){
		...
		if($_FILES["profile"]["error"] == 4 || $_FILES["cover"]["error"]==4){
			...
			if (($_FILES["profile"]["error"] == 0) && ($_FILES["cover"]["error"] == 0)){
					$fileTypePP = exif_imagetype($_FILES["profile"]["tmp_name"]);
					$fileTypeCP = exif_imagetype($_FILES["cover"]["tmp_name"]);
					$allowed = array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG);
					if ((!in_array($fileTypePP, $allowed))) { 
						$errorinput = 1;
					} elseif((!in_array($fileTypeCC, $allowed))){ 
						$errorinput = 1;
					} else {
						$upload_pp = 1;
						$upload_cp = 1;
		...
		if($upload_pp == 1){
			...
			$queryinsertcp = "UPDATE account SET profile_pic = '$path' WHERE acc_id = $user_id";
			$result = mysqli_query($dbconn,$queryinsertcp);
			move_uploaded_file($_FILES['profile']['tmp_name'],'images/pp_cover/' . $_FILES['profile']['name']);
		}
		if($upload_cp == 1){
			$path = $_FILES['cover']['name'];
			$queryinsertcp = "UPDATE account SET cover_photo = '$path' WHERE acc_id = $user_id";
			$result = mysqli_query($dbconn,$queryinsertcp);
			move_uploaded_file($_FILES['cover']['tmp_name'],'images/pp_cover/' . $_FILES['cover']['name']);
		}
		header("Location: people_profile.php");
	}
?>